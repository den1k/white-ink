(ns white-ink.utils.actions
  (:require [om.core :as om]
            ...
            [white-ink.utils.state :refer [save-note!
                                           ...
                                           update-cur-insert!
                                           persist-typed-text
                                           persist-scroll-offset-chan]]
            [white-ink.utils.styles.transition :as trans])
  (:require-macros [cljs.core.async.macros :as async]))

(defn start-actions-handler [{:keys [actions tasks]} app-state]
  (let [persist-scroll-offset (persist-scroll-offset-chan app-state)
        persist-insert-text (persist-typed-text app-state)]
    (async/go-loop []
                   (when-let [action-vec (<! actions)]
                     ((fn [action-vec]
                        (match [action-vec]
                               ...
                               [[:scroll-offset :reviewer el]] (put! persist-scroll-offset el)
                               ...
                               [[:persist-insert-text new-text]] (put! persist-insert-text new-text)
